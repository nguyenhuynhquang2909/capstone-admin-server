    ...
    async updateEatingSchedule(
        ...
        const sql = `
            ...
            WHERE 
                es.id = $1
        `;
        const existingSchedules = await this.eatingScheduleRepository.query(sql, [eatingScheduleId]);
        ...
        const updatedClassId = updateEatingScheduleDto.class_id ?? existingSchedule.class_id;
        const updatedStartTime = updateEatingScheduleDto.start_time ?? existingSchedule.start_time;
        const updatedEndTime = updateEatingScheduleDto.end_time ?? existingSchedule.end_time;
        ...
        const updatedNutrition = updateEatingScheduleDto.nutrition ?? existingSchedule.nutrition;
        const updatedLocationId = updateEatingScheduleDto.location_id ?? existingSchedule.location_id;
        ...
        const oldMediaIds = existingSchedules.map((schedule) => schedule.media_id).filter(Boolean);
        if (oldMediaIds.length > 0) {
            const deleteMealMediaSql = `
                DELETE FROM meal_media WHERE meal_id = $1
            `;
            await this.eatingScheduleRepository.query(deleteMealMediaSql, [eatingScheduleId]);
        ...
        if (newFiles && newFiles.length > 0) {
            const uploadedMedia = await this.mediaService.uploadMedia(newFiles, userId);
            for (const media of uploadedMedia) {
               const insertMealMediaSql = `
                INSERT INTO meal_media (meal_id, media_id) VALUES ($1, $2)
               `;
               await this.eatingScheduleRepository.query(insertMealMediaSql, [eatingScheduleId, media.id]);
               Logger.log(`Associated Media ID: ${media.id} with Eating Schedule ID: ${eatingScheduleId}`);
            }
        }
        ...
        const updatedSchedule = await this.eatingScheduleRepository.query(newSql, [eatingScheduleId]);
