...
export class StudentController {
  ...
  @Get('all-students')
  @Role('schoolAdmin')
  async getAllStudentsForSchool(@Headers('authorization') authHeader: string) {
    if (!authHeader) {
  ...
  @Get(':studentId')
  @Role('schoolAdmin')
  async getStudentProfile(@Headers('authorization') authHeader: string) {
    if (!authHeader) {
      throw new UnauthorizedException('Authorization header is missing');
    }
  ...
  async enrollNewStudent(
    ...
    const newStudent = await this.studentService.enrollNewStudent(
      enrollNewStudentDto,
      userId,
    );
    let media = [];
    if (files && files.files && files.files.length > 0) {
      ...
      for (const mediaItem of media) {
        await this.studentService.associateMediaWithStudent(
          newStudent.id,
          mediaItem.id,
        );
      }
